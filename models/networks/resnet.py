'''
ResNet from the pytorch torchvision library with the linear classifier removed for our Incremental Learning setting.

CifarResNet modify the normal ResNet to fit the CIFAR images (3x32x32) as proposed by [He et al., 2015].
...
class CifarResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if dilate:
            ...
            stride = 1
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = CifarDownsampling(planes)
...
def wide_resnet50_2(num_classes):
    return ResNet(Bottleneck, [3, 4, 6, 3], num_classes, width_per_group=64 * 2)
